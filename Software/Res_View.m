...
% RES_VIEW Graphical interface for viewing and analysing tracking data.
%   RES_VIEW opens an interactive GUI that allows you to load videos and their
%   tracking results (usually generated by behavioural analysis algorithms).
%
%   DESCRIPTION:
%   The software allows you to view the trajectory of a subject (e.g. rodents or
%   zebrafish) within an arena, manually or automatically correct
%   tracking errors (such as head-tail inversion) and generate statistical graphs.
%
%   MAIN FEATURES:
%   * Directory Loading: Selection of the folder containing the videos and .mat data.
%   * Visualisation: Video playback with Centroid, Head and Tail overlay.
%   * Smoothing: Application of moving averages to clean up noisy trajectories.
%   * Spatial Analysis: Generation of heatmaps, occupancy probability maps
%     by sector and angular histograms of head position.
%   * Kinematic Analysis: Calculation and display of speed with adjustable thresholds.
%   * Zone Management: Definition and loading of custom ROIs (Regions of Interest).
%   * Export: Export of processed data in .mat format and generation of videos
%     with rendered trajectories.
%
%   DATA REQUIREMENTS:
%   The function expects a specific folder structure where the tracking results
%   are located in subfolders “Results/Raw” relative to the video location,
%   with files named “_data.mat” and “Out_*.mat”.
%
%   USAGE:
%   Res_View           - Starts the interface.
%   h = Res_View       - Starts the interface and returns the handle to the figure.
%
%   See also: GUIDE, GUIDATA, VIDEOREADER, CONVHULL.
...
function Res_View_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = Res_View_OutputFcn(hObject, eventdata, handles)
...
function open_directory_2_Callback(hObject, eventdata, handles)
...
if isempty(handles.video_file_listbox.String)
    ...
    handles.video_file = cellstr(get(handles.video_file_listbox,'String'));
    ...
    handles.vidfilename = [handles.folder_name filesep handles.video_file{get(handles.video_file_listbox,'Value')}];
    ...
    handles.last_time_edit.String = num2str(handles.video.NumberOfFrames/handles.video.FrameRate);
...
try
    ...
    handles.track.Time=0:1/handles.video.FrameRate:(handles.video.NumberOfFrames-1)/handles.video.FrameRate;
    ...
    try
        ...
        handles.Centroid = zeros(2,handles.video.NumberOfFrames);
        handles.Centroid(:,i_first:i_last)=Tracked.track.Centroid;
        handles.Head = zeros(2,handles.video.NumberOfFrames);
        handles.Head(:,i_first:i_last)=Tracked.track.Head;
        handles.Tail = zeros(2,handles.video.NumberOfFrames);
...
if ~(editstr>=1 && editstr<=handles.video.NumberOfFrames-5)
...
function mobile_window_size_Callback(hObject, eventdata, handles)
...
% --- Executes during object creation, after setting all properties.
function mobile_window_size_CreateFcn(hObject, eventdata, handles)
% hObject    handle to mobile_window_size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function first_time_edit_Callback(hObject, eventdata, handles)
...
% --- Executes during object creation, after setting all properties.
function first_time_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to first_time_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
if ~(editstr>=first_time && editstr<=handles.video.NumberOfFrames)
...
Y1=round(min(handles.data.arena(:,2)));
Y2=round(max(handles.data.arena(:,2)));
[i_first,i_last] = getFrameBounds(handles);
D1=X2-X1;
D2=Y2-Y1;
...
h=convhull(Centroid(1,i_first:i_last)', Centroid(2,i_first:i_last)');
...
Xaxis=floor(handles.data.arena_x);
Yaxis=floor(handles.data.arena_y);
...
for i=i_first:length(Centroid)
    Centroid(1,i)=(Centroid(1,i)-X1)/D2*Xaxis;
    Centroid(2,i)=Yaxis-(Centroid(2,i)-Y1)/D1*Yaxis;
end
Vx=NaN(1,length(Centroid));
Vy=NaN(1,length(Centroid));
for i=i_first:length(Centroid)-1
...
win = win*handles.video.FrameRate;
...
data_export(handles,i_first,i_last,handles.vidfilename)
...
fn = round(str2double(get(handles.current_frame_edit,'String')));
i_last=handles.video.NumberOfFrames;

% Get Head coordinates
CurrFrame = rgb2gray(read(handles.video,fn));
...
writerObj.FrameRate = handles.video.FrameRate;
open(writerObj);
f=figure();
    [i_first,i_last] = getFrameBounds(handles);
...
for i = i_first:i_last
    Frame=rgb2gray(read(handles.video,i));
    imshow(Frame,[])
...
for i = i_first:i_last
    if handles.lvl<150
        CurrFrame=rgb2gray(read(handles.video,i));
    else
        CurrFrame=rgb2gray(read(handles.video,i));
