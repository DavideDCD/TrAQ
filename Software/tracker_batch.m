function tracker_batch(handles)

Directory =handles.video_dir_text.String;
cd(Directory)
vidfiles = dir(fullfile(Directory, '*.avi')); % cambiare estensione file se necessario

if isempty(vidfiles)==1
    vidfiles = dir(fullfile(Directory, '*.mp4')); % cambiare estensione file se necessario
end
if isempty(vidfiles)==1
    vidfiles = dir(fullfile(Directory, '*.mpg')); % cambiare estensione file se necessario
end
if isempty(vidfiles)==1
    vidfiles = dir(fullfile(Directory, '*.mpg')); % cambiare estensione file se necessario
end

nfiles = length(vidfiles);
DataFolder=[Directory filesep 'Results' filesep 'Raw'];
if ~exist(DataFolder,'dir')
    mkdir(DataFolder)
end

for i = 1 : nfiles
    clear Time Centroid Head Tail Area Axes Tracked Eccentricity EulerNumber ConvexHull
    File=vidfiles(i).name(1:end-4);
    try
        File=vidfiles(i).name(1:end-4);
        data_file_name=[Directory filesep 'Results' filesep 'Raw' filesep 'Data' filesep File '_data.mat'];
        load(data_file_name);
        flag=0;
    catch
        File=vidfiles(1).name(1:end-4);
        data_file_name=[Directory filesep 'Results' filesep 'Raw' filesep 'Data' filesep File '_data.mat'];
        load(data_file_name);
        flag=1;
    end
    vid_file=[Directory filesep vidfiles(i).name];
    video = VideoReader(vid_file);
    nFrames_tot = video.NumberOfFrames;
    FrameRate=video.FrameRate;
    Time=0:1/FrameRate:(nFrames_tot-1)/FrameRate;
    ...
    if color==4
        ...
        for i_frame = i_start:i_end
            ...
            if handles.lvl>100
                ...
                %             GreyThresh = max(max(CurrFrame))*data.GreyThresh;
                if data.arena==0
                ...
                %             GreyThresh = max(max(CurrFrame))*data.GreyThresh;
                if data.arena==0
                    CurrFrame=(single(CurrFrame-(Bkg)));
                else
                    CurrFrame=((single(CurrFrame-(Bkg))).^2).*F;
                end
            end
            BW = imerode(imbinarize(CurrFrame,GreyThresh),strel('disk',floor(Erosion)));
            [xhead,yhead,xtail,ytail,maxArea,cc,majoraxis,minoraxis,eccentricity,vertices,eulernumber,~,flag]=getcoordinates(BW,Area_th,Erosion);
            ...
            if get(handles.live_tracking, 'Value') == 1
                ...
                hold on
                plot(cc(1),cc(2),'r+')
                plot(xhead, yhead, 'r.','MarkerSize',15)
                plot(xtail, ytail, 'b.','MarkerSize',15)
                txt = {'Frame:',num2str(i_frame)};
                text(10,20,txt,'Color','white')
                hold off
        ...
        for i_frame = i_start:i_end
            ...
            if handles.lvl>100
                ...
                if data.arena==0
                    ...
                    CurrFrame=(single(CurrFrame-(Bkg))).*F;
                end
            else
                CurrFrame=(read(video,i_frame));
    ...
    StrFile_out=[DataFolder filesep 'Out_',File,'.mat'];
    save(StrFile_out, 'track')
    
    if flag ==1
        data_dir = [Directory filesep 'Results' filesep 'Raw' filesep 'Data'];
        data_file_name = [data_dir filesep File,'_data.mat'];
